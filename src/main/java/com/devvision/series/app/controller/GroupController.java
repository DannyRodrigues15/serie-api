...
import com.devvision.series.app.service.GroupService;

@CrossOrigin(origins = "*")
@RestController
@RequestMapping("/v1/groups")
public class GroupController extends Controller implements IController<GroupDTO> {
	@Autowired 
	...
	@PostMapping
	public ReturnRequest insert(@RequestHeader Map<String, String> headers, @Valid @RequestBody GroupDTO dto) {
		try {
			verifyToken(headers.get("token"));
	...
	@PutMapping("/{id}")
	public ReturnRequest update(@RequestHeader Map<String, String> headers, @Valid @PathVariable Long id, @RequestBody GroupDTO dto) {
		try {
			verifyToken(headers.get("token"));
